        ...
        <style>
          ...
          table,td,th{border: 1px solid green;margin: 0;pxpadding: 0px;}
            table { border-collapse: collapse;}
             td {width: 347px;height: 35px;color:white;text-align: center;font-size: 15px;padding-top: 0px;margin-top: 0px;}
              th {width: 341.5px;height: 40px;color:white;text-align: center;font-size: 20px;padding-top: 0px;margin-top: 0px;}
...
session.setAttribute("sub", usub);
 ...
 if(uname == null) {
      ...
      %>
        ...
        </div><div style="position: absolute;width: 77.7%;top: 200px;left: 175px;">
   ...
   // Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,ResultSet.CONCUR_READ_ONLY);
            ...
            ps=con.prepareStatement("select NAME,"+upsub+",ID from "+uclass+" order by ID");
            ...
            out.print("<form action='stu_insert' name='yfill'>");
            out.println("<input type='hidden' name ='type' value='edyfill' />");
            out.println("<table>");
            for(i=0;i<3;i++)
            {
                 out.println("<tr>");
                for(j=0;j<2;j++)
                { if(j==0)
                    out.print("<td style=' width:500px'>"+s1[i][j]+"</td>"); 
                  else
                {
                   // s1[i][j]="<input type=text name='"+i+"' style='width:500px'></input>";
                    out.print("<td style=' width:500px'><input type=text name='"+i+"' value='"+s1[i][j]+"' style='width:500px;height:100%;padding:0px:margin:0px;text-align:center;font-family: algerian '></input></td>");
                }
            ...
            out.println("</table>");
            out.print("<br><br>");
             out.print("<input type=submit value='SUBMIT' style='margin-left:478px;font-family:algerian'>");
              out.print("</form>");
